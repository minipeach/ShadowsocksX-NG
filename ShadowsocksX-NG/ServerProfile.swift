...
class ServerProfile: NSObject {
    ...
    func toDictionary() -> [String:AnyObject] {
        ...
        d["ServerPort"] = NSNumber(value:serverPort)
    ...
    func toJsonConfig() -> [String: AnyObject] {
        var conf: [String: AnyObject] = ["server": serverHost as AnyObject,
                                         "server_port": NSNumber(value: serverPort),
                                         "password": password as AnyObject,
                                         "method": method as AnyObject,]
        
        let defaults = UserDefaults.standard
        conf["local_port"] = NSNumber(value: UInt16(defaults.integer(forKey: "LocalSocks5.ListenPort")))
        conf["local_address"] = defaults.string(forKey: "LocalSocks5.ListenAddress") as AnyObject
        conf["timeout"] = NSNumber(value: UInt32(defaults.integer(forKey: "LocalSocks5.Timeout")))
        conf["auth"] = NSNumber(value: ota)
        if(!ssrObfs.isEmpty){
